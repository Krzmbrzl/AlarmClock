...
public class Starter {
			...
			display.asyncExec(new Runnable() {
				...
				public void run() {
					...
					if (parentShell.getLayout() instanceof StackLayout) {
						StackLayout layout = (StackLayout) parentShell.getLayout();
						...
						// create the clock
						AnalogClock clock = new AnalogClock(alarmShell, SWT.NONE, false);
						clock.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, true));

						// create composite for the alarm controls
						Composite alarmControls = new Composite(alarmShell, SWT.NONE);
						alarmControls.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, true));
						...
						Button terminateButton = new Button(alarmControls, SWT.PUSH);
						terminateButton.setFont(font);
						terminateButton.setText("Terminate");
						terminateButton.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, false));
						final Shell helper = parentShell;
						terminateButton.addListener(SWT.Selection, new Listener() {
						...
						nextButton.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, false));
				...
				public void run() {
					...
					if (alarms.size() > 0 && alarms.get(0).isActive()) {
						...
						if (hours > 48) {
							...
							// print hours until alarm
							message = "Next alarm in " + hours + " hours and " + minutes + " minutes";
						}
					}
	...
	protected static void setUpDisplaySaver(Display display) {
		displayOffTimer.schedule(new TimerTask() {
			...
			public void run() {
				try {
					new ProcessBuilder(new String[] { "xset", "dpms", "force", "off" }).start();
